namespace shoe_shop_productAPI.Helper
{
    public class StoreProceductLinks
    {
        public static string SP_GET_USER = "sp_get_user";
        public static string SP_REGISTER_USER = "sp_register_user";
        public static string SP_PRODUCTS = "sp_keysearch_product";
        public static string SP_CREATE_PRODUCTS = "sp_create_product";
        public static string SP_GET_ORDER_DETAIL = "sp_get_order_detail";
        public static string SP_GET_ORDER_DETAIL_BY_ID = "sp_get_order_detail_by_id";
        public static string SP_UPDATE_JWT_TOKEN = "sp_update_jwt_token";
        public static string SP_UPLOAD_IMAGE = "sp_upload_image";
        public static string SP_GET_PRODUCTS = "sp_get_products";
        public static string SP_DELETE_PRODUCTS = "sp_delete_product";
        public static string SP_UPDATE_PRODUCTS = "sp_update_product";

    }
}
